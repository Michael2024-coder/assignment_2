...
class TestGameBasic(unittest.TestCase):
    ...
    def test_pvp_play_simple(self, mock_roll, mock_input):
    ...
    def test_pvp_play_double_one(self, mock_roll, mock_input):
    ...
    def test_pvp_play_single_one(self, mock_roll, mock_input):
    ...
    def test_pvc_play_player_turn(self, mock_roll, mock_input):
    ...
    def test_pvc_play_ai_turn(self, mock_easy, mock_comp_roll):
    ...
    def test_pvc_play_single_one_player(self, mock_roll, mock_input):
    ...
    @patch("dice.game.Game.computer_roll")
    @patch("dice.intelligence.Intelligence.easy", return_value="n")
    def test_pvc_play_double_one_ai(self, mock_easy, mock_comp_roll):
        """Test that a double one in AI PvC turn resets score to 0."""
        self.game.current_player_pvc = "Jarvis AI"
        mock_comp_roll.return_value = {
            "is_double_one": True,
            "is_single_one": False,
    ...
    def test_pvc_play_must_reroll(self, mock_roll, mock_input):
    ...
    # ---------------- PvP pause and quit ----------------
    @patch(
        "builtins.input", side_effect=["p", "4", "y"]
    )
    @patch(
        "dice.game.Game.roll",
        return_value={
    ...
    def test_pvp_pause_quit(self, mock_roll, mock_input):
        """Test pausing and quitting during PvP turn resets scores and returns 'quit'."""
        self.game.current_player_pvp = "Player 1"
    ...
    @patch(
        "builtins.input", side_effect=["p", "3", "player123", "NewName", "n"]
    )
    @patch(
        "dice.game.Game.roll",
        return_value={
    ...
    def test_pvc_player_change_name(self, mock_roll, mock_input):
        """Test that a player can change name during PvC turn."""
        self.game.current_player_pvc = "Player"
        self.game.pvc_scores["Player"] = 0
        self.game.pvc_play("2", "1")
    ...
    # ---------------- PvC AI medium level decision to hold ----------------
    @patch("dice.game.Game.computer_roll")
    @patch("dice.intelligence.Intelligence.mid", return_value="n")
    def test_pvc_ai_mid_hold(self, mock_mid, mock_comp_roll):
        """Test AI decision at medium level to hold in PvC."""
        self.game.current_player_pvc = "Jarvis AI"
    ...
    # ---------------- New: double one PvP resets score ----------------
    def test_double_one_pvp_resets_score(self):
        """Test that double one in PvP resets player's score to 0."""
        self.game.current_player_pvp = "Player 1"
        self.game.pvp_scores["Player 1"] = 50
        self.game.double_one_pvp()
        self.assertEqual(self.game.pvp_scores["Player 1"], 0)

    # ---------------- New: double one PvC resets score ----------------
    def test_double_one_pvc_resets_score(self):
        """Test that double one in PvC resets player's score to 0."""
        self.game.current_player_pvc = "Player"
    ...
    def test_save_pvp1_updates_stats(self):
        """Test saving PvP1 stats updates histogram and highscore."""
    ...
    # ---------------- New: save PvC player wins ----------------
    def test_save_pvc1_player_wins(self):
        """Test saving PvC stats when player wins updates histogram and highscore."""
    ...
    def test_player_type_new(self, mock_input):
        """Test player_type method when creating a new player."""
    ...
    # ---------------- New: player_type existing ----------------
    @patch("builtins.input", side_effect=["2", "existingID"])
    def test_player_type_existing(self, mock_input):
        """Test player_type method when selecting an existing player."""
        self.game.player = MagicMock()
        self.game.histogram = MagicMock()
